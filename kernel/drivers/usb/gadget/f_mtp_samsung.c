...
static inline void req_put(struct mtpg_dev *dev, struct list_head *head,
...
static int mtpg_open(struct inode *ip, struct file *fp)
{
	...
	if (_lock(&the_mtpg->open_excl)) {
		printk(KERN_DEBUG "mtpg_open fn -- mtpg device busy\n");
		return -EBUSY;
	}
				...
				size_t count, loff_t *pos)
{
	...
	if (_lock(&dev->read_excl)) {
		printk(KERN_DEBUG "mtpg_read fn -- mtpg device busy\n");
		return -EBUSY;
	}
	...
	while (!((dev->online || dev->error) && dev->read_ready)) {
		...
		if (ret < 0) {
			...
			printk(KERN_DEBUG "mtp_read ret<0 (line %d)\n",
				__LINE__);
			return ret;
		}
	}
	...
	while (count > 0) {
		...
		if (dev->error) {
			r = -EIO;
			printk(KERN_DEBUG "**[%s]\t%d: dev->error so break r=%d\n",
				__func__, __LINE__, r);
			break;
		}
...
requeue_req:
			...
			if (ret < 0) {
				...
				printk(KERN_DEBUG "[%s] \t line %d, RETURN ERROR r = %d\n",
					__func__, __LINE__, r);
				goto fail;
			} else {
		...
		if (dev->read_count > 0) {
			...
			DEBUG_MTPR("[%s]copy_to_user:0x%x bytes"
				" on endpoint %p\n",
			...
			/* Updating the buffer size and
				returnung from mtpg_read */
			r = xfer;
				 ...
				 size_t count, loff_t *pos)
{
	...
	if (_lock(&dev->write_excl)) {
		DEBUG_MTPW("USB/Driver/f_mtp.c mtpg_write"
			" dev->write_exel _lock\n");
		return -EBUSY;
		}
	...
	while (count > 0) {
		if (dev->error) {
			r = -EIO;
			printk(KERN_DEBUG "* [%s]\t%d count > 0 but dev->error so break !\n",
					__func__, __LINE__);
			break;
		}
		...
		if (req != 0) {
			...
			if (ret < 0) {
				DEBUG_MTPW("mtpg_write after"
					" ep_queue ret < 0 brk\n");
				dev->error = 1;
				r = -EIO;
				DEBUG_MTPW("[%s]\t%d after ep_queue ret=%d"
					" so break return = %d\n",
					__func__, __LINE__, ret, r);
				break;
			}
...
static ssize_t interrupt_write(struct file *fd, const char __user *buf,
	size_t count)
{
	...
	if (!req) {
		printk(KERN_DEBUG "Alloc has failed\n");
		return -EIO;
	}

	if (_lock(&dev->wintfd_excl)) {
		printk(KERN_DEBUG "write failed on interrupt endpoint\n");
		return -EBUSY;
	}

	if (copy_from_user(req->buf, buf, count)) {
		printk(KERN_DEBUG "copy from user has failed\n");
...
static long  mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case MTP_ONLY_ENABLE:
		printk(KERN_DEBUG "[%s:%d] calling mtp_function_enable with %d\n",
	...
	case MTP_WRITE_INT_DATA:
		printk(KERN_DEBUG "[%s] \t %d MTP intrpt_Write\n",
			__func__, __LINE__);
		ret_value = interrupt_write(fd, (const char *)arg,
			MTP_MAX_PACKET_LEN_FROM_APP);
		if (ret_value < 0) {
			printk(KERN_DEBUG "[%s] \t %d interrupt-fd failed\n",
				__func__, __LINE__);
			status = -EIO;
		}	else {
			printk(KERN_DEBUG "[%s] \t %d interrupt fd success\n",
				__func__, __LINE__);
	...
	case SEND_RESET_ACK:
		/*req->zero = 1;*/
		req->length = 0;
		/*printk("[%s] SEND_RESET_ACK and usb_ep_queu 0"
			" data size = %d\tline = [%d]\n",
			__func__,size,__LINE__);*/
		status = usb_ep_queue(cdev->gadget->ep0, req, GFP_ATOMIC);
		if (status < 0)
				DEBUG_MTPB("[%s] Error at usb_ep_queue"
				"\tline = [%d]\n", __func__, __LINE__);
		break;
	case SET_SETUP_DATA:
		buf_ptr = (char *)arg;
	...
	case SET_ZLP_DATA:
		...
		req->length = 0;
		printk(KERN_DEBUG "[%s] SEND_ZLP_DATA and usb_ep_queu 0 data size = %d\tline = [%d]\n",
			__func__, size, __LINE__);
		status = usb_ep_queue(dev->bulk_in, req, GFP_ATOMIC);
		if (status < 0) {
			printk(KERN_DEBUG "[%s] Error at usb_ep_queue\tline = [%d]\n",
				__func__, __LINE__);
		} else {
			printk(KERN_DEBUG "[%s] usb_ep_queue passed and status = %d\tline = [%d]\n",
				__func__, __LINE__, status);
			status = 20;
		}
		break;
	case GET_HIGH_FULL_SPEED:
		printk(KERN_DEBUG "[%s] GET_HIGH_FULL_SPEED and \tline = [%d]\n",
			__func__, __LINE__);
...
static int __init mtpg_function_bind(struct usb_configuration *c,
	struct usb_function *f)
{
	...
	if (id < 0) {
		printk(KERN_DEBUG "Error in usb_string_id Failed !!!\n");
		return id;
	}
	...
	if (!ep) {
		printk(KERN_DEBUG "Error in usb_ep_autoconfig for fs IN DESC Failed !!!!!!!!!!\n");
		goto autoconf_fail;
	}
	...
	if (!ep) {
		printk(KERN_DEBUG "Error in usb_ep_autoconfig for fs OUT DESC Failed !!!!!!!!!!\n");
		goto autoconf_fail;
	}
	ep->driver_data = mtpg;		/* claim the endpoint */
	mtpg->bulk_out = ep;
	...
	if (!ep) {
		printk(KERN_DEBUG "Error in usb_ep_autoconfig for fs INT IN DESC Failed !!!!!!!!!!\n");
		goto autoconf_fail;
	}
	ep->driver_data = mtpg;
	mtpg->int_in = ep;
	...
	if (gadget_is_dualspeed(cdev->gadget)) {
		...
		printk(KERN_DEBUG "[%s] \tdual speed line = [%d]\n",
			__func__, __LINE__);
...
#ifdef CONFIG_USB_ANDROID_MTP_INIT_AFTER_ENABLE
	/*This is required for intializing Descriptors to NULL*/
	/*printk("[%s:%d] calling mtp_function_enable with"
...
autoconf_fail:
	printk(KERN_DEBUG "mtpg unable to autoconfigure all endpoints\n");
	rc = -ENOTSUPP;
out:
		...
		unsigned intf, unsigned alt)
{
	...
	ret = usb_ep_enable(dev->int_in, ep_choose(cdev->gadget,
		&int_hs_notify_desc, &int_fs_notify_desc));
	if (ret) {
		usb_ep_disable(dev->int_in);
		dev->int_in->driver_data = NULL;
		printk(KERN_DEBUG "USB/Driver/f_mtp.c Error in enabling the interrupt endpoint\n");
		return ret;
	}
	...
	ret = usb_ep_enable(dev->bulk_in, ep_choose(cdev->gadget,
		&hs_mtpg_in_desc, &fs_mtpg_in_desc));
	if (ret) {
		usb_ep_disable(dev->bulk_in);
		dev->bulk_in->driver_data = NULL;
		 printk(KERN_DEBUG "[%s] Enable Bulk-Out EP error! %d\n",
			__func__, __LINE__);
		 return ret;
	}
	...
	ret = usb_ep_enable(dev->bulk_out, ep_choose(cdev->gadget,
		&hs_mtpg_out_desc, &fs_mtpg_out_desc));
	if (ret) {
		usb_ep_disable(dev->bulk_out);
		dev->bulk_out->driver_data = NULL;
		printk(KERN_DEBUG "[%s] Enable Bulk-In EP error!!! %d\n",
			__func__, __LINE__);
		return ret;
	}
		...
		struct usb_request *req)
{
	...
	if (req->actual != USB_PTPREQUEST_CANCELIO_SIZE)	{
		DEBUG_MTPB("[%s] USB_PTPREQUEST_CANCELIO_SIZE"
			"\tline = [%d]\n", __func__, __LINE__);
		usb_ep_set_halt(ep);

	} else {
		memset(dev->cancel_io_buf, 0,
			USB_PTPREQUEST_CANCELIO_SIZE+1);
		memcpy(dev->cancel_io_buf, req->buf,
			USB_PTPREQUEST_CANCELIO_SIZE);
		/*Debugging*/
#ifdef DEBUG_MTP_SETUP
		for (i = 0; i < USB_PTPREQUEST_CANCELIO_SIZE; i++)
			DEBUG_MTPB("[%s] cancel_io_buf[%d] = %x"
			" \tline = [%d]\n", __func__, i,
			dev->cancel_io_buf[i], __LINE__);
#endif
...
/* soonyong.cho :  do nothing if we are disabled */
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		DEBUG_MTPB("[%s] USB_PTPREQUEST_CANCELIO"
			" \tline = [%d]\n", __func__, __LINE__);
		DEBUG_MTPB("[%s] \tline = [%d]  w_value = %x,"
			"w_index = %x, w_length = %x\n",
			__func__, __LINE__, w_value, w_index, w_length);
		/*if (w_value == 0x00 &&
			w_index == mtpg_interface_desc.bInterfaceNumber	&&
			w_length == 0x06)*/
		if (w_value == 0x00 && w_length == 0x06)	{
			DEBUG_MTPB("[%s] read USB_PTPREQUEST_CANCELIO data"
				" \tline = [%d]\n", __func__, __LINE__);
			value = w_length;
			...
			value = usb_ep_queue(cdev->gadget->ep0,
				req, GFP_ATOMIC);
			if (value < 0)
				printk(KERN_DEBUG "[%s] \tline = [%d] Error at usb_ep_queue\n",
					__func__, __LINE__);
		}
		return value;
		break;
...
int __init mtp_function_add(struct usb_configuration *c)
{
	...
	if (!mtpg) {
		printk(KERN_DEBUG "[%s] \t mtpg_dev_alloc memory"
			"  failed\n", __func__);
		return -ENOMEM;
	}
...
/*Test the switch*/
#ifndef CONFIG_USB_ANDROID_MTP_INIT_AFTER_ENABLE
	...
	if (rc != 0) {
		printk(KERN_DEBUG "Error in misc_register of mtpg_device Failed !!!\n");
		goto err_misc_register;
	}

	rc = usb_add_function(c, &mtpg->function);
	if (rc != 0) {
		printk(KERN_DEBUG "Error in usb_add_function Failed !!!\n");
...
err_misc_register:
	kfree(mtpg);
	printk(KERN_DEBUG "mtp gadget driver failed to initialize !!!\n");
	return rc;
}
