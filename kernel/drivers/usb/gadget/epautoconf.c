...
#endif
		if ((strcmp(ep->name, "ep5in") == 0) ||
			(strcmp(ep->name, "ep5out") == 0)){
			//printk(KERN_INFO "[usb] skip for shared ep5in, ep5out\n", __func__, __LINE__);
		}else if (ep_matches (gadget, ep, desc)){
			return ep;
		}
...
)
{
    ...
    type = desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK;
      ...
      /* single buffering is enough */
          ...
          ep = find_ep (gadget, "ep5in");
          if (ep && shared_ep_matches (gadget, ep, desc,5))
          return ep;
