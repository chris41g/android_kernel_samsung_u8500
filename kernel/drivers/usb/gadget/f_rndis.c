...
#define CSY_DBG(fmt, args...) \
	printk(KERN_INFO "[f_rndis] %s:%d "fmt, __func__, __LINE__, ##args)
...
rndis_iad_descriptor = {
	...
	.bFunctionSubClass = 1,
	.bFunctionProtocol =	3,
#elif defined(CONFIG_USB_ANDROID_SAMSUNG_RNDIS_WITH_MS_COMPOSITE)
	.bFunctionClass =	0xe0,
...
//	spin_lock(&dev->lock);
		...
		int i;
		printk(KERN_INFO "usb rndis_command_complete buf=");
		for(i=0; i < req->length; i++)
		{
			printk(KERN_INFO "%02x ", ((char*)req->buf)[i]);
		}
		printk("\n");
#endif
