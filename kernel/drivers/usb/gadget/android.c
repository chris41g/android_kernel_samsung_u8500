...
/* nina.lee : refer product id and config string of usb from 'arch/arm/mach-ux500/include/mach/devices.h' */
...
#ifdef CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE
/* jh46_ahn  - Lockup issue after removing USB cable when enabling and disabling USB tethering */
static struct notifier_block notifier_block;
#endif

/* Default vendor and product IDs, overridden by platform data */
#ifdef CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE
#define VENDOR_ID		SAMSUNG_VENDOR_ID	/* SAMSUNG */
/* soonyong.cho : default product id refered as <plat/devs.h> */
#define PRODUCT_ID		SAMSUNG_DEBUG_PRODUCT_ID
#else /* Original VID & PID */
#define VENDOR_ID		ANDROID_VENDOR_ID
#define PRODUCT_ID		ANDROID_PRODUCT_ID
...
/* soonyong.cho : Samsung string default product id refered as <plat/devs.h> */
	...
	[STRING_SERIAL_IDX].s = "U8500_Android",
#else /* Original */
...
#else /* original */
	.label		= ANDROID_NO_DEBUG_CONFIG_STRING,   	//.label		= "android",
#endif /* CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE */
		...
		struct usb_function *f)
{
	...
	CSY_DBG2("find name=%s, num_functions =%d \n",name,count);
	for (i = 0; i < count; i++) {
	...
	if (p) {
		for (i = 0; i < count; i++, p++) {
			if(p->mode == mode) {
				...
				CSY_DBG("set_enable_functions: functions =%s, num_functions =%d  \n",p->functions, p->num_functions);
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if(enable) {
		...
		if (!strcmp(f->name, "phonet")) {
			ret = set_product(dev, USBSTATUS_PHONET);
		}
#endif
		...
		if (!strcmp(f->name, "rndis") && dev->debugging_usb_mode)
			ret = set_product(dev, USBSTATUS_ADB);
#ifdef CONFIG_USB_ANDROID_PHONET
		if (!strcmp(f->name, "phonet") && dev->debugging_usb_mode)
			ret = set_product(dev, USBSTATUS_ADB);
#endif
		else
...
static void samsung_enable_function(int mode)
{
...
#endif
#ifdef CONFIG_USB_ANDROID_PHONET
		case USBSTATUS_PHONET: /* do not save usb mode */
			CSY_DBG_ESS("mode = USBSTATUS_PHONET (0x%x)\n", mode);
			ret = set_product(dev, USBSTATUS_PHONET);
			break;
#endif
	...
	else if((mode != USBSTATUS_ADB) && (mode != USBSTATUS_VTP) 
		             && (mode != USBSTATUS_ASKON) && (mode != USBSTATUS_PHONET)) {
...
static ssize_t tethering_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* skip tethering on when the phonet enable*/
	if (a_dev->requested_usb_mode == USBSTATUS_PHONET)
		return size;
	...
	/* added for checking disconnect cable when disabling tethering */
		if (a_dev->cdev->mute_switch)
			a_dev->cdev->mute_switch = 2;
	}
...
/* jh46.ahn :  sysfs for to show status of usb config
 *                Path (/sys/devices/platform/android_usb/phonet)
 */
static ssize_t phonet_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct android_dev *a_dev = _android_dev;
	int value = -1;
	...
	if(a_dev->cdev) {
		...
		CSY_DBG("Fail to show phonet switch. dev->cdev is not valid\n");
	}
	return sprintf(buf, "%d\n", value);
}

/* jh46.ahn : sysfs for to change status of phonet switch
 *                Path (/sys/devices/platform/android_usb/phonet)
 */
static ssize_t phonet_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int value;
	struct android_dev *a_dev = _android_dev;
	sscanf(buf, "%d", &value);
	...
	if (value) {
		CSY_DBG_ESS("Enable phonet\n");
		samsung_enable_function(USBSTATUS_PHONET);
	}
	else {
		CSY_DBG_ESS("Disable phonet\n");
		if(a_dev->debugging_usb_mode)
			samsung_enable_function(USBSTATUS_ADB);
		else
		samsung_enable_function(a_dev->current_usb_mode);

		/* added the disconnect intend when disabling phonet */
		a_dev->cdev->mute_switch = 0;
	}

	return size;
}

/* attribute of sysfs for phonet switch */
static DEVICE_ATTR(phonet, S_IRUGO|S_IWUSR|S_IWGRP,
		   phonet_switch_show, phonet_switch_store);
...
#endif
#ifdef CONFIG_USB_ANDROID_PHONET
			case USBSTATUS_PHONET:
				return sprintf(buf, "[UsbMenuSel] PHONET\n");
#endif
			case USBSTATUS_ADB:
...
{
	struct android_dev *a_dev = _android_dev;
//	printk("[ANDROID] usb_switch_notify_clients event = 0x%08lx\n",action);

	if (action & USB_SWITCH_DISCONNECTION_EVENT) {
		printk(KERN_INFO "[ANDROID] a_dev->cdev->mute_switch = %d\n",
			a_dev->cdev->mute_switch);
		if ((a_dev->cdev->mute_switch == 1
				&& a_dev->cdev->products->product_id
				== SAMSUNG_RNDIS_PRODUCT_ID)
				|| a_dev->cdev->mute_switch == 2)
			switch_set_state(&a_dev->cdev->sdev, 0);
	}
...
static int android_probe(struct platform_device *pdev)
{
	...
	CSY_DBG2("\n");
#ifdef CONFIG_USB_ANDROID_SAMSUNG_COMPOSITE
	CSY_DBG2("Apply Samsung composite ! \n");
#else
	CSY_DBG2("Apply Android original composite!\n");
#endif
...
/* jh46.ahn add for NTrace*/
		if (device_create_file(&pdev->dev, &dev_attr_phonet) < 0)
			CSY_DBG("Failed to create device file(%s)!\n", dev_attr_phonet.attr.name);
