...
struct lsm303dlh_a_data {
	...
	int emul;  /* emulate sensor for sysfs */
...
static ssize_t lsm303dlh_a_store_emul(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	...
	ddata->emul = (short)val;
	if (!val && ddata->emul)
		regulator_disable(ddata->regulator);
	else if (val && !ddata->emul)
		regulator_enable(ddata->regulator);
	mutex_unlock(&ddata->lock);
...
static ssize_t lsm303dlh_a_show_emul(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t lsm303dlh_a_store_x(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t lsm303dlh_a_show_x(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t lsm303dlh_a_store_y(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t lsm303dlh_a_show_y(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t lsm303dlh_a_store_z(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t lsm303dlh_a_show_z(struct device *dev, 
					struct device_attribute *attr,
					char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
...
static DEVICE_ATTR(emul, S_IWUGO | S_IRUGO,
		lsm303dlh_a_show_emul, lsm303dlh_a_store_emul);
static DEVICE_ATTR(x, S_IWUGO | S_IRUGO,
		lsm303dlh_a_show_x, lsm303dlh_a_store_x);
static DEVICE_ATTR(y, S_IWUGO | S_IRUGO,
		lsm303dlh_a_show_y, lsm303dlh_a_store_y);
static DEVICE_ATTR(z, S_IWUGO | S_IRUGO,
		lsm303dlh_a_show_z, lsm303dlh_a_store_z);
...
static struct attribute *lsm303dlh_a_attributes[] = {
	&dev_attr_emul.attr,	/* emulator sensor */
	&dev_attr_x.attr, 	/* emulator sensor */
	&dev_attr_y.attr,	/* emulator sensor */
	&dev_attr_z.attr,	/* emulator sensor */
	&dev_attr_data.attr,
	&dev_attr_range.attr,
	&dev_attr_mode.attr,
	&dev_attr_rate.attr,
