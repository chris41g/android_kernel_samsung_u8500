...
{
  if(MPUACC_DEBUG_CFG) {
    printk(KERN_INFO "requested_sensors:%ld\n", mldl_cfg->requested_sensors);
//  printk("ignore_system_suspend:%d\n", mldl_cfg->ignore_system_suspend);
    printk(KERN_INFO "addr:%d\n", mldl_cfg->addr);
    printk(KERN_INFO "int_config:%d\n", mldl_cfg->int_config);
    printk(KERN_INFO "ext_sync:%d\n", mldl_cfg->ext_sync);
    printk(KERN_INFO "full_scale:%d\n", mldl_cfg->full_scale);
    printk(KERN_INFO "dmp_enable:%d\n", mldl_cfg->dmp_enable);
    printk(KERN_INFO "fifo_enable:%d\n", mldl_cfg->fifo_enable);
    printk(KERN_INFO "dmp_cfg1:%d\n", mldl_cfg->dmp_cfg1);
    printk(KERN_INFO "dmp_cfg2:%d\n", mldl_cfg->dmp_cfg2);
    printk(KERN_INFO "gyro_power:%d\n", mldl_cfg->gyro_power);
    printk(KERN_INFO "gyro_is_bypassed:%d\n", mldl_cfg->gyro_is_bypassed);
    printk(KERN_INFO "dmp_is_running:%d\n", mldl_cfg->dmp_is_running);
    printk(KERN_INFO "gyro_is_suspended:%d\n", mldl_cfg->gyro_is_suspended);
    printk(KERN_INFO "accel_is_suspended:%d\n", mldl_cfg->accel_is_suspended);
    printk(KERN_INFO "compass_is_suspended:%d\n", mldl_cfg->compass_is_suspended);
    printk(KERN_INFO "pressure_is_suspended:%d\n", mldl_cfg->pressure_is_suspended);
    printk(KERN_INFO "gyro_needs_reset:%d\n", mldl_cfg->gyro_needs_reset);
  }
}
...
mpu_accel_activate_device(struct mpuaccel_data *data, int enable)
{
  ...
  if(MPUACC_DEBUG){
    printk(KERN_INFO "activate device:%d, result=%d\n", enable, result);
  }
...
mpu_accel_input_work_func(struct work_struct *work)
{
  ...
  if(MPUACC_DEBUG){
    printk(KERN_INFO "________________START____________________\n");
  }
  if(MPUACC_DEBUG_CFG) {
      ...
      if(data_is_valid) {
        ...
        if(MPUACC_DEBUG) {
          if(data_is_from_mpu==1)           
            printk(KERN_INFO "MPU_ACCEL:[%d][%d][%d]\n",
			    accel[0], accel[1], accel[2]);
          else
            printk(KERN_INFO "ACCEL:[%d][%d][%d]\n",
			    accel[0], accel[1], accel[2]);
        }        
#ifdef MPUACC_USES_CACHED_DATA  
        ...
        if(MPUACC_DEBUG) {
          printk(KERN_INFO "input device is updated\n");
        }
      }
    }
  }
  if(MPUACC_DEBUG){
    printk(KERN_INFO "________________END____________________\n");
  }  
  mpu_accel_mutex_lock(data);
  enable = atomic_read(&data->enable);
  mpu_accel_mutex_unlock(data);
...
mpu_accel_enable(struct mpuaccel_data *data)
{
  ...
  if(MPUACC_DEBUG){
    printk(KERN_INFO "mpu_accel_enable : %d\n", atomic_read(&data->enable));
  }
  ...
  if(atomic_read(&data->enable) != 1) {
    ...
    if(MPUACC_DEBUG){
      printk(KERN_INFO "mpu_accel_enable : enabled\n");
    }  

    if(mldl_cfg->accel_is_suspended == 1) {
      if(MPUACC_DEBUG){
        printk(KERN_INFO "mpu_accel_enable : turn on accel\n");
      }      
...
mpu_accel_disable(struct mpuaccel_data *data)
{
  ...
  if(MPUACC_DEBUG){
    printk(KERN_INFO "mpu_accel_disable : %d\n", atomic_read(&data->enable));
  }
  ...
  if(atomic_read(&data->enable) != 0) {
    ...
    if(MPUACC_DEBUG){
      printk(KERN_INFO "mpu_accel_disable : disabled\n");
    }

    if(mldl_cfg->accel_is_suspended == 1) {
      if(MPUACC_DEBUG){
        printk(KERN_INFO "mpu_accel_disable : turn off accel\n");
      }      
...
int mpu_accel_get_cached_data(unsigned char* cache)
{
  ...
  if(pThisData!=NULL) {
    if(pThisData->device_is_on==1) {
      ...
      if(1) {
        printk(KERN_INFO "cached data:[%d][%d][%d][%d][%d][%d]\n", 
