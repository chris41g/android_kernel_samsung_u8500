...
#define COL 8
static u8 gp2a_original_image_00B[COL][2] = {
	/*  {Regster, Value} */
		       ...
		       const char *buf, size_t count)
{
	...
	if (data->proximity_enabled && !value) {	/* Prox power off */
		...
		regulator_disable(regul.regulator_vcc);
		regulator_disable(regul.regulator_vio);
...
int lightsensor_get_adc(struct gp2a_data *data)
{
	...
	if ((D0_data == 0 || D1_data == 0)\
		...
		lx = lx_prev;
#ifdef DEBUG
pr_debug("Data range over so ues prev_lx value=%d D0=%d, D1=%d mode=%d\n",\
lx, D0_data, D1_data, data->lightsensor_mode);
#endif
		return lx;
	} else {
...
#ifdef DEBUG
		pr_debug("D0=%d, D1=%d, lx=%d mode=%d a=%d, b=%d prev_lx=%d\n",
			D0_raw_data, D1_raw_data, lx, data->lightsensor_mode,
...
error_setup_reg_prox:
	kfree(gp2a);
out_rm_regulator:
	if (regul.regulator_vcc) {
#ifdef SENSORS_REGULATOR_CONTROL
		regulator_disable(regul.regulator_vcc);
#endif
	}
	if (regul.regulator_vio) {
#ifdef SENSORS_REGULATOR_CONTROL
		regulator_disable(regul.regulator_vio);
#endif
	}
