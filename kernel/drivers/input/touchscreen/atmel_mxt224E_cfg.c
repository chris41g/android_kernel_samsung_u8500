...
uint8_t mxt_touch_threshold = 40;
...
int mxt_get_object_values(struct mxt_data *mxt, int obj_type)
{
	...
	if (error < 0) {
		dev_err(&client->dev, "mxt_read_byte failed!\n");
		return -EIO;
	}
...
int mxt_multitouch_config(struct mxt_data *mxt)
{
	...
	if (mxt->rotate)
		touchscreen_config.orient       = T9_ORIENT_180;
	else
		touchscreen_config.orient       = T9_ORIENT_NORM;
...
int mxt_comc_config(struct mxt_data *mxt)
{
	...
	obj_size = MXT_GET_SIZE(MXT_SPT_COMMSCONFIG_T18);		touchscreen_config.orient       = T9_ORIENT_180;
...
int mxt_noisesuppression_t48_config(struct mxt_data *mxt)
{
	...
	noisesuppression_t48_config.freq_0            = T48_FREQ_0;
	noisesuppression_t48_config.freq_1            = T48_FREQ_1;
	noisesuppression_t48_config.freq_2            = T48_FREQ_2;
	noisesuppression_t48_config.freq_3            = T48_FREQ_3;
	noisesuppression_t48_config.mffreq_2          = T48_MFFREQ_2;
	noisesuppression_t48_config.mffreq_3          = T48_MFFREQ_3;
	noisesuppression_t48_config.nlgain            = T48_NLGAIN;
	noisesuppression_t48_config.nlthr             = T48_NLTHR;
	noisesuppression_t48_config.gclimit           = T48_GCLIMIT;
	...
	noisesuppression_t48_config.gcinvalidthr      = T48_GCINVALIDTHR;
...
int mxt_config_settings(struct mxt_data *mxt)
{
	...
	if (mxt_one_touch_gesture_config(mxt) < 0)    /* T24 */
		return -1;
...
static void bootloader_status(u8 value)
{
	...
	if (mxt_debug)
		printk(KERN_INFO "bootloader status: %s (0x%02X)\n",
			str, value);
}
...
int mxt_load_firmware(struct device *dev, const char *fn)
{
	...
	mxt_reset_chip(mxt, RESET_TO_BOOTLOADER);
	...
	printk(KERN_INFO "Updating progress: ");
	pos += 2;
...
try_to_resend_the_last_frame:
	...
	printk(KERN_INFO "note: You may need to reset this target.\n");
