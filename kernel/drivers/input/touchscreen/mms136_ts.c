...
#define MMS_VSC_ENTER			0X01
#define MMS_VSC_CM_DELTA		0X02
#define MMS_VSC_CM_ABS			0X03
#define MMS_VSC_EXIT			0X05
#define MMS_VSC_INTENSITY		0X04
#define MMS_VSC_RAW			0X06
#define MMS_VSC_REFER			0X07

/*
#define	MMS_VSC_CMD2			0xA0
#define	MMS_VSC_CMD2_RESULT_SIZE	0xAE
#define	MMS_VSC_CMD2_RESULT		0xAF
#define	MMS_VSC_CMD2_ENTER_TEST_MODE	0x40
#define	MMS_VSC_CMD2_TEST_CM_DELTA	0x41
#define	MMS_VSC_CMD2_GET_PIXEL_CM_DELTA	0x42
#define	MMS_VSC_CMD2_TEST_CM_ABS	0x43
#define	MMS_VSC_CMD2_GET_PIXEL_CM_ABS	0x44
#define	MMS_VSC_CMD2_EXIT_TEST_MODE	0x4F
*/
...
struct mms_ts_info {
	...
	int				irq;
	struct finger_state		finger[MAX_FINGERS];
...
static void report_touch_data(struct mms_ts_info *info, bool force_clear)
{
	int i;

	for (i = 0; i < MAX_FINGERS; i++) {
		if (info->finger[i].s == -1)
			continue;
		...
		if (force_clear) {
			...
			dev_info(&info->client->dev,
				"up/f[%d]: %3d, %3d (%3d,%3d)\n",
				i, info->finger[i].x, info->finger[i].y,
				info->finger[i].w, info->finger[i].s);
		}
		...
		input_report_abs(info->input_dev_ts,
			 ABS_MT_WIDTH_MAJOR, info->finger[i].w);
		input_report_abs(info->input_dev_ts,
			 ABS_MT_TOUCH_MAJOR, info->finger[i].s);
		input_mt_sync(info->input_dev_ts);
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < sz; i += FINGER_EVENT_SZ) {
		...
		if (type == TOUCH_SCREEN) {
			int last_s = info->finger[id].s;
			info->finger[id].x = tmp[2] | ((tmp[1] & 0xf) << 8);
			info->finger[id].y =
					tmp[3] | (((tmp[1] >> 4) & 0xf) << 8);
			info->finger[id].w = tmp[4];
			info->finger[id].s = (action) ? tmp[5] : 0;
			need_report_ts = true;

			if (info->pdata->invert_x) {
				info->finger[id].x = info->pdata->max_x -
							info->finger[id].x;
				if (info->finger[id].x < 0)
					info->finger[id].x = 0;
			}
			if (info->pdata->invert_y) {
				info->finger[id].y = info->pdata->max_y -
							info->finger[id].y;
				if (info->pdata->invert_y < 0)
					info->pdata->invert_y = 0;
			}

			if (!action) {
				dev_info(&client->dev,
					"up  [%d]: %3d, %3d (%3d,%3d)\n", id,
					info->finger[id].x, info->finger[id].y,
					info->finger[id].w, info->finger[id].s);
			} else if ((last_s == -1)  && action) {
				dev_info(&client->dev,
					"down[%d]: %3d, %3d (%3d,%3d)\n", id,
					info->finger[id].x, info->finger[id].y,
					info->finger[id].w, info->finger[id].s);
...
static void fw_update(void *device_data)
{
	...
	ret = mms_ts_fw_update(info, BUILT_IN, ISC_FLASH, true);
				  ...
				  const struct i2c_device_id *id)
{
	...
	input_set_abs_params(input_dev_ts, ABS_MT_TRACKING_ID, 0,
						(MAX_FINGERS - 1), 0, 0);
	...
	input_set_abs_params(input_dev_ts, ABS_MT_TOUCH_MAJOR, 0,
						MAX_PRESSURE, 0, 0);
	input_set_abs_params(input_dev_ts, ABS_MT_WIDTH_MAJOR, 0,
						MAX_WIDTH, 0, 0);
	...
	__set_bit(EV_KEY, input_dev_tk->evbit);
	for (i = 0; i < info->pdata->key_nums; i++)
...
static int mms_ts_enable(struct mms_ts_info *info)
{
	...
	msleep(5); /* usleep_range(3000, 5000); */
	info->enabled = true;
	enable_irq(info->irq);
out:
...
static int mms_ts_disable(struct mms_ts_info *info)
{
	...
	msleep(12); /* usleep_range(10000, 12000); */
	info->enabled = false;
out:
	mutex_unlock(&info->lock);
	return 0;
