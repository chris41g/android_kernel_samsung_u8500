	...
	for (i = 0; i < data->cap_info.max_finger; i++) {
		...
		if (force_clear) {
			...
			if (prev_exist && cur_up) {
				...
				y = cur->coord[i].y;
				w = 0;
				cur->coord[i].sub_status &= 0xFE;
				goto report;
			}
			if (cur_exist || cur_move || cur_down) {
				x = cur->coord[i].x;
				y = cur->coord[i].y;
				w = cur->coord[i].width;
				if (!w) {
...
report:
		...
		input_report_abs(data->input_dev_ts, ABS_MT_WIDTH_MAJOR, w);
		input_report_abs(data->input_dev_ts, ABS_MT_TOUCH_MAJOR, w);
		input_mt_sync(data->input_dev_ts);
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = bt404_ts_read_data(client, addr, (u8 *)&buff, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s: err: rd (key event)\n", __func__);
		goto out;
	}

	for (i = 0; i < data->pdata->num_buttons; i++) {
		if (data->pdata->button_map[i] == KEY_BACK) {
			state = (int)((buff >> i) & 0x1);
			break;
		}
	}

	dev_info(&client->dev, "back key state: %d (%d:%X)\n", state, i, buff);
out:
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < data->pdata->num_buttons; i++) {
		if (data->pdata->button_map[i] == KEY_MENU) {
			state = (int)((buff >> i) & 0x1);
			break;
		}
	}
	dev_info(&client->dev, "menu key state: %d (%d:%X)\n", state, i, buff);
out:
	return snprintf(buf, sizeof(int), "%x\n", state);
}
					...
					const struct i2c_device_id *i2c_id)
{
	...
	input_set_abs_params(input_dev_ts, ABS_MT_TRACKING_ID, 0,
					REAL_SUPPORTED_FINGER_NUM - 1, 0, 0);
