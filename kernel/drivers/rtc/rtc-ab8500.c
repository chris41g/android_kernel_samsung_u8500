...
/* RtcCalibration bits */
#define RTC_CAL_MASK			0xff
#define RTC_CAL_DATA			0x96
 ...
 */
static ktime_t rtc_tm_to_ktime(struct rtc_time tm)
{
	time_t time;
...
static int __devinit ab8500_rtc_probe(struct platform_device *pdev)
{
	...
	err = abx500_mask_and_set_register_interruptible(&pdev->dev, AB8500_RTC,
		AB8500_RTC_CALIB_REG, RTC_CAL_MASK, RTC_CAL_DATA);
	if (err < 0) {
		dev_err(&pdev->dev, "Rtc Calibration failed\n");
		return err;
	}
#endif
