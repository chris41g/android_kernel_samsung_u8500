...
{
	printk(KERN_INFO "c6xdigio_pwmo_insn_read %x\n", insn->n);
	return insn->n;
}
			   ...
			   struct comedi_devconfig *it)
{
	...
	iobase = it->options[0];
	printk(KERN_INFO "comedi%d: c6xdigio: 0x%04lx\n", dev->minor, iobase);
	if (!request_region(iobase, C6XDIGIO_SIZE, "c6xdigio")) {
		printk(KERN_INFO "comedi%d: I/O port conflict\n", dev->minor);
		return -EIO;
	}
	dev->iobase = iobase;
	...
	irq = it->options[1];
	if (irq > 0)
		printk(KERN_INFO "comedi%d: irq = %u ignored\n", dev->minor, irq);
	else if (irq == 0)
		printk(KERN_INFO "comedi%d: no irq\n", dev->minor);
...
static int c6xdigio_detach(struct comedi_device *dev)
{
	...
	printk(KERN_INFO "comedi%d: c6xdigio: remove\n", dev->minor);
