...
static ssize_t ste_sysfs_vibrator_on_off(struct device* dev,
		struct device_attribute* attr, char* buf);


static DEVICE_ATTR(vibrator, S_IRUGO, ste_sysfs_vibrator_on_off, NULL);


static ssize_t ste_sysfs_vibrator_on_off(struct device* dev,
		struct device_attribute* attr, char* buf)
{
	struct ste_vibra_speed speed_left ;
	struct ste_vibra_speed speed_right;
	...
	case 0:
		/* Turn ON vibrator */
		speed_left.positive = 0;
		speed_left.negative = 100;
		speed_right.positive = 0;
		speed_right.negative = 100;	
		break;
		
	case 1:
		/* Turn OFF vibrator */		
		speed_left.positive = 0;
		speed_left.negative = 0;
		speed_right.positive = 0;
		speed_right.negative = 0;		
		break;
	}
...
static int ste_audio_io_drv_probe(struct platform_device *pdev)
{
	...
	error = device_create_file(&pdev->dev, &dev_attr_vibrator);
	if(error < 0) {
		dev_err(&ste_audio_io_device->dev,
			"device_create_file failed err = %d",
			error);
		goto ste_audio_io_misc_deregister;		
	}
...
{
	device_remove_file(&pdev->dev, &dev_attr_vibrator);
