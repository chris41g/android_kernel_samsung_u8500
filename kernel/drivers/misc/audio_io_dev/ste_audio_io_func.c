					...
					struct device *dev)
{
	...
	if (channel_index & e_CHANNEL_1) {
		...
		/* Class D control */
		error = HW_ACODEC_MODIFY_WRITE(CLASS_D_PATH_CONTROL_REG,
							LEFT_HF_HIGH_VOL, 0);
		if (0 != error) {
			dev_err(dev,
				"Class D path control %d",
									error);
			return error;
		}
					...
					struct device *dev)
{
	...
	initialVal_AD = HW_REG_READ(DIGITAL_AD_CHANNELS_ENABLE_REG);  /*DIGITAL_DA_CHANNELS_ENABLE_REG*/
					...
					struct device *dev)
{
	...
	if (channel_index & e_CHANNEL_1) {
		...
		error = HW_ACODEC_MODIFY_WRITE(AD_ALLOCATION_TO_SLOT6_7_REG,
							DATA_FROM_AD_OUT1, 0x0F);
		if (0 != error) {
	...
	error = ste_audio_io_set_lin_gain(channel_index,0,0,0,dev);
	if (0 != error) {
		dev_err(dev, "nste_audio_io_set_lin_gain%d", error);
		return error;
	}

	error = ste_audio_io_enable_fade_lin(dev);
	if (0 != error) {
		dev_err(dev, "ste_audio_io_enable_fade_lin%d", error);
		return error;
	}

	error = ste_audio_io_unmute_lin(channel_index,NULL,dev );
	if (0 != error) {
		dev_err(dev, "ste_audio_io_unmute_lin%d", error);
		return error;
	}
					...
					struct device *dev)
{
	...
	error = ste_audio_io_mute_lin(channel_index, dev);
	if (0 != error) {
		dev_err(dev, "ste_audio_io_mute_lin%d", error);
		return error;
	}
