...
static int mloader_fw_send(struct dbx500_ml_fw *fw_info)
{
	...
	const unsigned long map_size = 0x4000;
	/*- 369241 -*/
	...
	{
		unsigned long copy_size = map_size;
		if (copy_offset + map_size > size)
		{
			copy_size = fw->size - copy_offset;
			ioaddr = ioremap(phys_start + copy_offset, PAGE_ALIGN(copy_size));
		}
		else
		{
			ioaddr = ioremap(phys_start + copy_offset, copy_size);
		}
	if (!ioaddr) {
		dev_err(&mloader_priv->pdev->dev,
	...
	vaddr = ioaddr + (fw_info->offset & ~PAGE_MASK);
		...
		memcpy_toio(vaddr, fw_data, copy_size);
	iounmap(ioaddr);
