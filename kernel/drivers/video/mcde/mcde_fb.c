...
/* When LCD split issue occur in godin project, it need to use
 * DDR 50% operation.
 */
...
static struct struct_frame_buf_mark  frame_buf_mark = {
	...
	.p_fb   = 0,
	.resX   = 480,    // it has dependency on h/w
	.resY   = 800,     // it has dependency on h/w
	.bpp    = 24,      // it has dependency on h/w
	.frames = 2
};
//}} Mark for GetLog - 1/2
...
static void early_suspend(struct early_suspend *data)
{
	...
	/* Remove APE & DDR OPP requirement */
	if (mfb->mcde_opp_requested) {
		prcmu_qos_remove_requirement(PRCMU_QOS_DDR_OPP,
				"mcde_fb");
		mfb->mcde_opp_requested = false;
	}
#endif
...
#if REQUEST_APE_DDR_OPP
	/* Request 50% APE & DDR OPP */
	if (mfb->mcde_opp_requested == false) {
		if (prcmu_qos_add_requirement(PRCMU_QOS_DDR_OPP,
				"mcde_fb", 50)) {
			printk("pcrm_qos_add DDR failed \n");
		}
		mfb->mcde_opp_requested = true;
	}
#endif
...
static int apply_var(struct fb_info *fbi, struct mcde_display_device *ddev)
{
	...
	if (ddev) {
		...
		if (!fmt) {
			dev_vdbg(&(ddev->dev), "check_var failed fmt\n");
			return -EINVAL;
		}
...
//{{ Mark for GetLog - 2/2
	frame_buf_mark.p_fb= (void*)fbi->fix.smem_start ;
	frame_buf_mark.resX= fbi->var.xres ;
	frame_buf_mark.resY= fbi->var.yres ;
	frame_buf_mark.bpp= fbi->var.bits_per_pixel ;
//{{ Mark for GetLog - 2/2
