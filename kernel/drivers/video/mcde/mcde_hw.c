...
extern int b2r2_suspend(struct platform_device *pdev, pm_message_t state);
extern int b2r2_resume(struct platform_device *pdev);
...
/*+ FDE */
/* [donghyun.jo] Fix for greenish image in video playback (STE ticket #349576)*/
	} else if (port->type == MCDE_PORTTYPE_DPI) {
/*		printk("***FDE  ovlyfmt:%d     pixelfmt:%d  \n",ovly->pix_fmt, port->pixel_format); */
...
static void update_overlay_registers(struct mcde_chnl_state *chnl,
			u8 idx, struct ovly_regs *regs,
			...
			enum mcde_display_rotation rotation)
{
	...
	static int opp_requested = false;

	if (chnl->ovly0->regs.enabled &&
			chnl->ovly1->regs.enabled &&
			!opp_requested) {
		/* Request 100% APE & DDR OPP */
		if (prcmu_qos_add_requirement(PRCMU_QOS_DDR_OPP,
				"mcde_hw", 100)) {
			dev_warn(&mcde_dev->dev, "pcrmu_qos_add DDR failed\n");
		}
		if (prcmu_qos_add_requirement(PRCMU_QOS_APE_OPP,
				"mcde_hw", 100)) {
			dev_warn(&mcde_dev->dev, "pcrmu_qos_add APE failed\n");
		}
		dev_vdbg(&mcde_dev->dev, "APE & DDR OPP set to 100\n");
		opp_requested = true;

	} else if ((!chnl->ovly0->regs.enabled ||
			!chnl->ovly1->regs.enabled) &&
			opp_requested) {
		/* Remove APE & DDR OPP request */
		prcmu_qos_remove_requirement(PRCMU_QOS_DDR_OPP, "mcde_hw");
		prcmu_qos_remove_requirement(PRCMU_QOS_APE_OPP, "mcde_hw");
		dev_vdbg(&mcde_dev->dev, "APE & DDR OPP set to default\n");
		opp_requested = false;
	}
	...
	/* PIXELFETCHERWATERMARK max = 128 words of 64 bits, shared between
	 * all overlays. The UI overlay is in ARGB_32bits and the video
	 * overlay is in RGB_24bits maximum (see liboverlay for more details)
	 *                 128x64 => 160x32 + 128x24
	 * Note: These settings have to be adjusted in dual-screen mode */
	if (idx == 0)
		pixelfetchwtrmrklevel = 160;
	else
		#ifdef CONFIG_DISPLAY_GENERIC_DSI_PRIMARY
		pixelfetchwtrmrklevel = 128;
		#else
		pixelfetchwtrmrklevel = 160;
		#endif

	dev_vdbg(&mcde_dev->dev, "%s: idx %d, bpp %d, wtrmrk %d, enabled %d\n",
			__func__, idx, regs->bits_per_pixel,
			pixelfetchwtrmrklevel, regs->enabled);

	if ((idx == 1) && (regs->bits_per_pixel > 24))
		dev_warn(&mcde_dev->dev, "%s: ovl1>24bpp. Please adjust wmrk\n",
				__func__);
	...
	MCDE_OVL0CR_COLCCTRL(regs->col_conv) |
		...
		#else
		MCDE_OVL0CR_MAXOUTSTANDING_ENUM(16_REQ) |
		#endif
...
static void wait_for_flow_disabled(struct mcde_chnl_state *chnl)
{
	...
	case MCDE_CHNL_A:
		for (i = 0; i < MCDE_FLOWEN_MAX_TRIAL; i++) {
			...
			/* Use udelay instead of msleep to limit the
			 * risk of re-scheduling that creates
			 * artifacts in DPI ram-less mode.
			 * Note hr timers or a thread in rt could be
			 * better solutions than the blocking udelay
			 * TODO Do the same update for Chnl B & C */
			udelay(1000);
		}
		break;
	case MCDE_CHNL_B:
				...
				struct mcde_video_mode *video_mode)
{
	...
	/* DPI LCD Mode */
		if (chnl_id == MCDE_CHNL_A) {
			mcde_wreg(MCDE_SYNCHCONFA,
				MCDE_SYNCHCONFA_HWREQVEVENT_ENUM(ACTIVE_VIDEO) |
				MCDE_SYNCHCONFA_HWREQVCNT(video_mode->yres-1) |
				MCDE_SYNCHCONFA_SWINTVEVENT_ENUM(ACTIVE_VIDEO) |
				MCDE_SYNCHCONFA_SWINTVCNT(video_mode->yres-1));
		} else if (chnl_id == MCDE_CHNL_B) {
			mcde_wreg(MCDE_SYNCHCONFB,
				MCDE_SYNCHCONFB_HWREQVEVENT_ENUM(ACTIVE_VIDEO) |
				MCDE_SYNCHCONFB_HWREQVCNT(video_mode->yres-1) |
				MCDE_SYNCHCONFB_SWINTVEVENT_ENUM(ACTIVE_VIDEO) |
				MCDE_SYNCHCONFB_SWINTVCNT(video_mode->yres-1));
		}
	}
	...
	if (regs->roten) {
		...
		mcde_wreg(MCDE_ROTACONF + chnl_id * MCDE_ROTACONF_GROUPOFFSET,
			...
			#ifdef CONFIG_MCDE_DISPLAY_S6E63MN0_DPI/*janice lcd*/
			MCDE_ROTACONF_STRIP_WIDTH_ENUM(32PIX) |
			#else
...
#ifndef CONFIG_MCDE_DISPLAY_DPI_MAIN
	schedule_delayed_work(&hw_timeout_work,
					msecs_to_jiffies(MCDE_SLEEP_WATCHDOG));
#endif /* CONFIG_MCDE_DISPLAY_DPI_MAIN */
