					...
					struct b2r2_work_buf *out_buf)
{
	...
	case B2R2_BLT_FMT_YUV422_PACKED_SEMIPLANAR_MB_STE:
		if ((req->user_req.flags & B2R2_BLT_FLAG_SOURCE_FILL) != 0) {
			...
			node->node.GROUP0.B2R2_CIC |= B2R2_CIC_IVMX;
			if (req->user_req.flags & B2R2_BLT_FLAG_FULL_RANGE_YUV) {
				node->node.GROUP15.B2R2_VMX0 =
				 ...
				 struct b2r2_work_buf *out_buf)
{
	...
	case B2R2_BLT_FMT_Y_CB_Y_CR:
		 ...
		 */
		if (req->user_req.flags & B2R2_BLT_FLAG_FULL_RANGE_YUV) {
			node->node.GROUP15.B2R2_VMX0 = B2R2_VMX0_YVU_TO_RGB_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX1 = B2R2_VMX1_YVU_TO_RGB_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX2 = B2R2_VMX2_YVU_TO_RGB_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX3 = B2R2_VMX3_YVU_TO_RGB_601_FULL_RANGE;
		} else {
			node->node.GROUP15.B2R2_VMX0 = B2R2_VMX0_YVU_TO_RGB_601_STANDARD;
			node->node.GROUP15.B2R2_VMX1 = B2R2_VMX1_YVU_TO_RGB_601_STANDARD;
			node->node.GROUP15.B2R2_VMX2 = B2R2_VMX2_YVU_TO_RGB_601_STANDARD;
			node->node.GROUP15.B2R2_VMX3 = B2R2_VMX3_YVU_TO_RGB_601_STANDARD;
		}
		break;
	case B2R2_BLT_FMT_CB_Y_CR_Y:
		/* Set up IVMX */
		...
		node->node.GROUP0.B2R2_CIC |= B2R2_CIC_IVMX;
		if (req->user_req.flags & B2R2_BLT_FLAG_FULL_RANGE_YUV) {
			node->node.GROUP15.B2R2_VMX0 = B2R2_VMX0_YUV_TO_RGB_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX1 = B2R2_VMX1_YUV_TO_RGB_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX2 = B2R2_VMX2_YUV_TO_RGB_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX3 = B2R2_VMX3_YUV_TO_RGB_601_FULL_RANGE;
		} else {
			node->node.GROUP15.B2R2_VMX0 = B2R2_VMX0_YUV_TO_RGB_601_STANDARD;
			node->node.GROUP15.B2R2_VMX1 = B2R2_VMX1_YUV_TO_RGB_601_STANDARD;
			node->node.GROUP15.B2R2_VMX2 = B2R2_VMX2_YUV_TO_RGB_601_STANDARD;
			node->node.GROUP15.B2R2_VMX3 = B2R2_VMX3_YUV_TO_RGB_601_STANDARD;
		}
		break;
	case B2R2_BLT_FMT_24_BIT_YUV888:
	case B2R2_BLT_FMT_32_BIT_AYUV8888:
				  ...
				  struct b2r2_work_buf *in_buf)
{
	...
	if (yuv_planar_dst) {
		...
		if (req->user_req.flags & B2R2_BLT_FLAG_FULL_RANGE_YUV) {
			node->node.GROUP15.B2R2_VMX0 = B2R2_VMX0_RGB_TO_YUV_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX1 = B2R2_VMX1_RGB_TO_YUV_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX2 = B2R2_VMX2_RGB_TO_YUV_601_FULL_RANGE;
			node->node.GROUP15.B2R2_VMX3 = B2R2_VMX3_RGB_TO_YUV_601_FULL_RANGE;
		} else {
			node->node.GROUP15.B2R2_VMX0 = B2R2_VMX0_RGB_TO_YUV_601_STANDARD;
			node->node.GROUP15.B2R2_VMX1 = B2R2_VMX1_RGB_TO_YUV_601_STANDARD;
			node->node.GROUP15.B2R2_VMX2 = B2R2_VMX2_RGB_TO_YUV_601_STANDARD;
			node->node.GROUP15.B2R2_VMX3 = B2R2_VMX3_RGB_TO_YUV_601_STANDARD;
		}
