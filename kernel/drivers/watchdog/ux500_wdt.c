...
extern u32 sec_debug_enable;
extern u32 sec_debug_enable_user;
...
static bool default_on;
module_param(default_on, bool, 0);
MODULE_PARM_DESC(default_on, "Enable watchdog at module start up");
...
static inline int wdog_kernel_kicker_init(struct platform_device *pdev)
{
	/*
	...
	wdog_kick_wq = create_rt_workqueue("ux500_wdog");
	if (!wdog_kick_wq) {
...
static inline int wdog_kernel_kicker_init(struct platform_device *pdev)
{
	...
	if (default_on) {
		prcmu_enable_a9wdog(wdog_id);
		wdt_en = true;
		dev_info(&pdev->dev, "enabled by default\n");
	}
...
{
	if (sec_debug_enable == 0 && sec_debug_enable_user == 0) {
		int ret;
		...
		ret = wdog_kernel_kicker_init(pdev);
		if (ret < 0)
			return ret;
...
#if !defined(CONFIG_UX500_WATCHDOG_KERNEL_KICKER)
