...
static int ab8500_fg_add_vcap_sample(struct ab8500_fg *di, int sample)
{
	...
	pr_info("Added vcap: %d, average: %d, nsamples: %d\n",
...
static void ab8500_fg_fill_i_sample(struct ab8500_fg *di, int sample)
{
	...
	pr_info("Filled curr sampe: 0, pre_sample : %d\n", sample);
	avg->pos = 0;
	avg->nbr_samples = 1;
	avg->sum = 0;
	avg->avg = 0;
...
static void ab8500_fg_add_i_sample(struct ab8500_fg *di, int sample)
{
	...
	pr_info("Added curr sample: %d, diff:%d, average: %d, high_diff_nbr: %d\n",
		...
		bool always)
{
	...
	if (i > WAIT_FOR_INST_CURRENT_MAX) {
		dev_info(di->dev, "Inst curr reading took too long, %d times\n",
			i);
		pr_info("returned uncompensated vbat\n");
		if (!always)
			return -1;
		di->vbat = vbat / i;
		return di->vbat;
		...
		bool always)
{
	...
	vbat_ret = ab8500_comp_fg_bat_voltage(di, false);
	if (vbat_ret == -1)
		return -1;
...
static int ab8500_fg_calc_cap_discharge_fg(struct ab8500_fg *di)
{
	...
	*/
...
#endif
	pr_info("Using every %d Vbat sample Now on %d loop\n",
	...
	if (++di->skip_add_sample >= di->n_skip_add_sample) {
		...
		pr_info("[CURR VAR] high_diff_nbr : %d\n",
			di->inst_cur.high_diff_nbr);

		pr_info("[DIFF CAP] FG_avg : %d, VB_avg : %d, diff_cap : %d\n",
			...
			pr_info("Adding voltage based samples to avg: %d\n",
				di->vbat_cap.avg);
...
static void ab8500_fg_algorithm_discharging(struct ab8500_fg *di)
{
	...
	case AB8500_FG_DISCHARGE_INITMEASURING:
		...
		if (di->init_cnt >
			...
			ab8500_fg_coulomb_counter(di, true);
			pr_info("Filling vcap with %d mah, avg was %d\n",
	...
	case AB8500_FG_DISCHARGE_READOUT:
...

		{
			...
			if (vbat_cap != -1) {
				...
				pr_info("Average voltage based capacity is: "
						"%d mah Now %d mah, [%d %%]\n",
					di->vbat_cap.avg, mah, vbat_cap / 10);
			} else
				pr_info("Ignoring average voltage based capacity\n");
		}
