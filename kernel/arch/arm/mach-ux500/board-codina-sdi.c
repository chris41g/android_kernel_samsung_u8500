		...
		unsigned char power_mode)
{
	...
	case MMC_POWER_ON:
		...
		gpio_direction_output(TXS0206_EN_CODINA_R0_0, 1);
		udelay(100);
		break;
	case MMC_POWER_OFF:
...
static void suspend_resume_handler_sdi2(struct mmc_host *host, bool suspend)
{
   if (suspend) {
	...
	gpio_set_value(MEM_LDO_EN_CODINA_R0_0, 0);
   } else {
...
/* BCM */
static int wifi_gpio_reset	= WLAN_RST_N_CODINA_R0_0;
static int wifi_gpio_irq	= WL_HOST_WAKE_CODINA_R0_0;

static void codina_wifi_init(void)
{
	int32_t status = 0;
...
static void codina_sdi2_init(void)
{
	...
	status = gpio_request(MEM_LDO_EN_CODINA_R0_0, "eMMC_EN");

	gpio_direction_output(MEM_LDO_EN_CODINA_R0_0, 1);
	gpio_set_value(MEM_LDO_EN_CODINA_R0_0, 1);
