...
static int display_initialized_during_boot_regu = (int)false;

static int __init early_startup_graphics_regu(char *p)
{
	if (get_option(&p, &display_initialized_during_boot_regu) != 1)
		display_initialized_during_boot_regu = false;
	return 0;
}

early_param("startup_graphics", early_startup_graphics_regu);
#endif
...
#ifdef CONFIG_MCDE_DISPLAY_DPI_MAIN
	/* If the B2R2_MCDE regu is already powered ON during the boot. 
	 * Note: It is important to not switch it ON again to prevent 
	 * glitches on the LCD DPI boot splash screen */
	if(display_initialized_during_boot_regu)
	{
		printk(KERN_INFO "startup_graphics detected => Set regulator B2R2_MCDE enabled\n"); 
		u8500_regulator_info[U8500_REGULATOR_SWITCH_B2R2_MCDE].is_enabled = true;
	}
#endif
