...
extern bool prcmu_is_mcdeclk_on(void);
...
#define MAX_STATE_DETERMINE_LOOP_TIME 100000 /* usec */
	...
	if (!power_state_req && prcmu_is_mcdeclk_on()) {
		power_state_req = 1;
		printk(KERN_ERR "cpuidle : wrong power_state_req value because it's LCD ON(MCDECLK_ON)\n");
	}
