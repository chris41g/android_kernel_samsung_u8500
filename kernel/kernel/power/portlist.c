...
struct ipc_pwr_ape_state_evt_type {
ipc_hdr_type hdr;
unsigned char state;
} __attribute__((__packed__));

union ipc_pwr_ape_state_type {
ipc_hdr_type hdr;
...
#ifdef DEBUG_PORT_INFO
	for ( index = 0; index < read_byte ; index++) {
		printk(KERN_INFO "%c",*(readline+index));
	}
#endif
...
int writePacketToFile(char *abuf, int length)
{
	...
	if (fp == NULL) {
	 ...
	 pr_info("[%s]request byte length = %d,  return byte = %d\n",
			__func__, length, ret);
...
#if defined(CONFIG_U8500_SHRM_SVNET)
char *hdlcFrame;
int  TxApstateToCP(unsigned char apstate)
{
	union ipc_pwr_ape_state_type ape_state;
	int hdlc_size;

	 ape_state.hdr.msg_seq = 0x0;
	 ape_state.hdr.ack_seq = 0xFF;
	 ape_state.hdr.main_cmd = 0x01; /* Power Commands */
	 ape_state.hdr.sub_cmd = 0x08; /* AP State Event Message */
	 ape_state.hdr.cmd_type = 0x05; /* Event */
	 ape_state.hdr.len = sizeof(union ipc_pwr_ape_state_type);

	 ape_state.ape_state_evt.state = (unsigned char)apstate;

	/* 7F(1) + HDLC header length + ipc packet length + 1 + 7E(1) */
	hdlc_size = 1 + 3 + ape_state.hdr.len + 1;

	hdlcFrame = kmalloc(hdlc_size, GFP_KERNEL);
	if (hdlcFrame == NULL)
		return -1;

	hdlcFrame[0] = 0x7F;
	/* Length except for 7F, 7E */
	hdlcFrame[1] =  ((unsigned short)(ape_state.hdr.len + 3) & 0x00ff);
	hdlcFrame[2] = ((unsigned short)(ape_state.hdr.len + 3) >> 8) & 0x00ff;
	hdlcFrame[3] = 0x00;

	memcpy(hdlcFrame + 4, (char *)&ape_state, ape_state.hdr.len);

	/* end packet(1byte) */
	hdlcFrame[4 + ape_state.hdr.len] = 0x7E;

#ifdef DEBUG_AP_STATE
	int i = 0;
	for ( ; i < hdlc_size; i++) {
		if (i % 16 == 0)
			pr_info("\n");

		pr_info("%02x ", *(hdlcFrame+i));
	}
#endif
...
int tx_ap_state(unsigned char apstate)
{
	int length = 0;
	int ret = 0;

	length = TxApstateToCP(apstate);

	if (hdlcFrame != NULL)
		ret = writePacketToFile(hdlcFrame, length);
