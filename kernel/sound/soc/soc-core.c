...
static int __init snd_soc_init(void)
{
	...
	if (!debugfs_create_file("codecs", 0444, debugfs_root, NULL,
				 &codec_list_fops))
		pr_info("ASoC: Failed to create CODEC list debugfs file\n");

	if (!debugfs_create_file("dais", 0444, debugfs_root, NULL,
				 &dai_list_fops))
		pr_info("ASoC: Failed to create DAI list debugfs file\n");

	if (!debugfs_create_file("platforms", 0444, debugfs_root, NULL,
				 &platform_list_fops))
		pr_info("ASoC: Failed to create platform list debugfs file\n");
#endif
