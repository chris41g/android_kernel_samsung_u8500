					    ...
					    size_t bytes, int dma_flag)
{
	...
	if (bytes_transmit < 0) {
		printk(KERN_INFO "error in transfer\n");
		return -1;
	}
						      ...
						      tdm8_ch_mode)
{
	...
	if (ENABLE == lpbk_state) {
		...
		if (status) {
			printk(KERN_INFO "failed in getting acodec playback open\n");
			return -1;
		}
		status = u8500_acodec_open(I2S_CLIENT_MSP1, 1);
		if (status) {
			printk(KERN_INFO "failed in getting acdoec capture open\n");
			return -1;
		}
...
static int i2sdrv_remove(struct i2s_device *i2s)
{
	...
	printk(KERN_INFO "Remove of I2S gets called\n");
	return 0;
}
...
static int check_device_id()
{
	...
	if (((data & 0xF0) == 0x10) || ((data & 0xF0) == 0x11)) {
		...
		printk(KERN_INFO "ERROR: AB8500 hardware detected is CUT1x\n");
		return -ENODEV;
#endif
	} else {
#ifndef CONFIG_U8500_AB8500_ED
		/* ED version */
		printk(KERN_INFO "ERROR: AB8500 hardware detected is EarlyDrop\n");
		return -ENODEV;
#endif
	}
	return 0;
...
static int __init u8500_acodec_init(void)
{
	...
	if (status < 0) {
		printk(KERN_INFO "Unable to register i2s driver\n");
		return status;
	}
